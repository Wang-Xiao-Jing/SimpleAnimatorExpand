...
public class Interpolation {
  public static <T> T interpolation(KeyFrame<T>[] keyframes, T out, float time) {
    ...
    if (kf1 != null && kf2 != null) {
      ...
      switch (kf2.getMode()) {
        ...
        case CATMULLROM ->
          out = kf2.catmullRomInterpolation(kf0.getPost(), kf1.getPost(), kf2.getPost(), kf3.getPost(), localT);
  ...
  public static Vector2f linerInterpolation(Vector2f p1, Vector2f p2, float delta) {
    return new Vector2f(p1.x * (1.0F - delta) + p2.x * delta, p1.y * (1.0F - delta) + p2.y * delta);
  }
  ...
  public static Vector3f linerInterpolation(Vector3f p1, Vector3f p2, Vector3f out, float delta) {
    return out.set(p1.x * (1.0F - delta) + p2.x * delta, p1.y * (1.0F - delta) + p2.y * delta, p1.z * (1.0F - delta) + p2.z * delta);
  }
